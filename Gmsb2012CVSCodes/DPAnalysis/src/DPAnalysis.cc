...
// $Id: DPAnalysis.cc,v 1.3 2013/02/12 02:56:20 tambe Exp $
//
//
...
bool DPAnalysis::HEBeamHaloMatch(Handle<HBHERecHitCollection> hhit, vector<const reco::Photon*> & selectedPhotons, const EventSetup &iSetup){
 ...
 // const EcalRecHitCollection *rechit = EBrecHitColl.product();
    ...
    for ( size_t i=0; i< selectedPhotons.size() ; i++ ) {
      ...
      double heRho  = -1 ;
      double  hetime = 99999.;
      double heEnergy = 999999.;
      double heXYradius = 9999.;
      double   hedphi  = 99.;
  ...
  // double scPhoEnergy = selectedPhotons[i]->superCluster()->energy();
   ...
   for (HBHERecHitCollection::const_iterator hhIt = hhit->begin(); hhIt != hhit->end(); hhIt++){
      ...
      Global3DPoint hbhePosition = hbhe_cell->getPosition();

      //only looking in HE members
      if (hbheDetId == 2){
       double herhEnergy = hhIt->energy();
       double herhTime = hhIt->time();
       double herhPhi = hbhePosition.phi();
       double heX = hbhePosition.x();
       double heY = hbhePosition.y();
       double heZ = hbhePosition.z();
       double radius = sqrt(heX*heX+heY*heY);
       double rho  = sqrt( (heX*heX) + (heY*heY) + (heZ*heZ) );
       LorentzVector HEp4( heX,heY, heZ, rho) ; 

       //compare phi of highest Et supercluster in event with phi's of all he rechits
      double  dPhi = ROOT::Math::VectorUtil::DeltaPhi( HEp4,   selectedPhotons[i]->p4() );
      double  AbsdPhi = fabs(scPhoPhi-herhPhi);   // Delta Phi between Selected Photon and HE rechit Position
        //phi wrapping
        if (AbsdPhi > TMath::Pi()){AbsdPhi = TMath::Pi()*2 - AbsdPhi;}
        if (dPhi < 0.01 ) Hehalomatch = true;
